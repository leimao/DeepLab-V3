...
def train(network_backbone, pre_trained_model=None, model_dir=None, log_dir='data/logs/deeplab/'):
    def validate(best_mIoU, valset_size, track_valset_size=False):
        val_loss_total = 0
        ...
        while True:
            try:
                ...
                num_pixels_union, num_pixels_intersection = count_label_prediction_matches(lbls, predictions, n_classes)
                ...
                p_bar.close()
                break
        mIoU = mean_intersection_over_union(num_pixels_union_total, num_pixels_intersection_total)
        val_loss_ave = val_loss_total / valset_size / batch_size
        print('Validation loss: {:.4f} | mIoU: {:.4f}'.format(val_loss_ave, mIoU))
        if mIoU > best_mIoU:
            best_mIoU = mIoU
    ...
    img_means_64 = img_means.astype(np.float64)
    ...
    best_mIoU, valset_size = validate(0, 0, track_valset_size=True)
    ...
    for i in range(n_epochs):
        ...
        while True:
            try:
                imgs, lbls = fetch_batch(model.sess.run(train_data), augment=True, min_scale_factor=0.25, max_scale_factor=1.0, img_means=img_means_64)
        ...
        best_mIoU, _ = validate(best_mIoU, valset_size)
