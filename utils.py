...
'''
def atrous_spatial_pyramid_pooling(inputs, scope, filters = 256, reuse = None):
    ...
    with tf.variable_scope(scope, reuse = reuse):
        ...
        # Atrous 1x1
        aspp1x1 = tf.layers.conv2d(inputs = inputs, filters = filters, kernel_size = (1, 1), padding = 'same', activation = None)
        # Atrous 3x3, rate = 6
        aspp3x3_1 = tf.layers.conv2d(inputs = inputs, filters = filters, kernel_size = (3, 3), padding = 'same', dilation_rate = (6, 6), activation = None)
        # Atrous 3x3, rate = 12
        aspp3x3_2 = tf.layers.conv2d(inputs = inputs, filters = filters, kernel_size = (3, 3), padding = 'same', dilation_rate = (12, 12), activation = None)
        # Atrous 3x3, rate = 18
        aspp3x3_3 = tf.layers.conv2d(inputs = inputs, filters = filters, kernel_size = (3, 3), padding = 'same', dilation_rate = (18, 18), activation = None)
        ...
        image_feature = tf.image.resize_bilinear(images = image_feature, size = [resize_height, resize_width], align_corners = True)

        # Merge Poolings
        outputs = tf.concat(values = [aspp1x1, aspp3x3_1, aspp3x3_2, aspp3x3_3], axis = 3)
        outputs = tf.layers.conv2d(inputs = outputs, filters = filters, kernel_size = (1, 1), padding = 'same', activation = None)
